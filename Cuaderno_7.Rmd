...
```{r}
model_1 <- lm(MO ~ Ca, data = df) 
summary(model_1)
```

$$Y_{MO} = 1.449 + 0.089X_{Ca}$$

```{r}
ggplot(df, aes(y = MO, x = Ca)) +
  geom_point()+
...
```{r}
model_3 <- lm(MO ~ Ca + CICE, data = df)
summary(model_3)
```

$$Y_{MO} = 1.359 + 0.090X_{Ca} + 0.001z$$
...
$$Y_{MO} = \alpha_0 + \lambda W Y_{MO} + u\\
u = \rho W u + \epsilon$$

**Si $\rho$ = 0, u = $\epsilon$**

$$Y_{MO} = \alpha_0 + \lambda W Y_{MO} + \epsilon$$
